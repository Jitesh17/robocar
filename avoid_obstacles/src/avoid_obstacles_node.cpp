...
void Listener::chatterCallback(const avoid_obstacles::BoundingBoxes::ConstPtr& msg)
{
  ...
  for(int i = 0; i< num ;i++){
    int number=i;
    string only="bottle";
    string m=msg->bounding_boxes[i].class_name.c_str();
    if(msg->bounding_boxes[i].ymax<400 && m==only.c_str())
    {
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    int y_thresh = 200;
    float max_speed =400;
    msg_angle.data = 0;//30*cos(x);
    float speed = max_speed/(1+exp(0.5*(box.real_y_max-y_thresh)));
    msg_speed.data = floor(speed);
